...
var (
	...
	ErrCantRemoveItemCart = errors.New("failed to remove item cart")  
	ErrCantGetItem = errors.New("failed to get item")  
...
func InstantBuyer(ctx context.Context, prodCollection, userrCollection *mongo.Collection, productID primitive.objectID, userID string)error
{
	id, err := primitive.ObjectIDFromHex(UserID)
	if err!= nil {
        log.Println(err)
